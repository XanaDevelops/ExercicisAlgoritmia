...
public interface Backtracking {
     ...
     * 
     * @ord L'ordre de complexitat d'aquest algorisme és aproximadament O(2^n), on n és la 
     ...
     * @base  El cas base es produeix quan ja no hi ha elements en el conjunt numèric per explorar. En aquest punt, es verifica 
     * si la suma del subconjunt actual és igual al valor objectiu M. Si és així, s'afegeix el subconjunt actual a la llista de 
     * resultats. Aquest és el punt on la recursió es deté i comença a retrocedir.
     * 
     * @general  El cas general es produeix quan encara hi ha elements en el conjunt numèric per explorar. En aquest cas, l'algorisme
     * realitza una elecció en cada pas, considerant si incloure o no l'element actual en el subconjunt. Es fa una crida recursiva
     * per explorar totes dues opcions.
